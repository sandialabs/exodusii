...
class ParallelExodusFile:
    ...
    @property
    def storage_type(self) -> _CallableString:
        """Floating-point storage type.

        Returned as a callable string for legacy compatibility, so both
        ``exo.storage_type`` and ``exo.storage_type()`` work.
        """

        return _CallableString(self._files[0].storage_type)
    ...
    def get_displ_variable_names(self, default: Any = None):
        """Legacy API: displacement variable names."""

        names = self.displacement_variable_names()
        return names if names else default

    def get_displ(self, time_step: int, default: Any = None):
        """Legacy API: nodal displacements at one-based time step."""

        names = self.displacement_variable_names()
        if not names:
            return default

        return self.displacements(time=time_step - 1)
    ...
    # ---------------------------------------------------------------------
    # Legacy-compatible method API
    # ---------------------------------------------------------------------

    def title_legacy(self) -> str:
        """Legacy helper for adapters that need method-style title access."""

        return self.title

    def num_dimensions(self) -> int:
        """Legacy API: number of spatial dimensions."""

        return self.dimension

    def num_nodes(self) -> int:
        """Legacy API: number of logical nodes."""

        return self.node_count

    def num_edges(self) -> int:
        """Legacy API: number of logical edges."""

        return self.edge_count

    def num_faces(self) -> int:
        """Legacy API: number of logical faces."""

        return self.face_count

    def num_elems(self) -> int:
        """Legacy API: number of logical elements."""

        return self.element_count

    def num_blks(self) -> int:
        """Legacy API: number of element blocks."""

        return self.element_block_count
    ...
    def num_node_sets(self) -> int:
        """Legacy API: number of node sets."""

        return self.node_set_count

    def num_side_sets(self) -> int:
        """Legacy API: number of side sets."""

        return self.side_set_count

    def num_edge_blk(self) -> int:
        """Legacy API: number of edge blocks."""

        return 0

    def num_edge_sets(self) -> int:
        """Legacy API: number of edge sets."""

        return 0

    def num_face_blk(self) -> int:
        """Legacy API: number of face blocks."""

        return 0

    def num_face_sets(self) -> int:
        """Legacy API: number of face sets."""

        return 0

    def num_elem_sets(self) -> int:
        """Legacy API: number of element sets."""

        return 0

    def num_elem_maps(self) -> int:
        """Legacy API: number of element maps."""

        return 0

    def num_node_maps(self) -> int:
        """Legacy API: number of node maps."""

        return 0

    def num_edge_maps(self) -> int:
        """Legacy API: number of edge maps."""

        return 0

    def num_face_maps(self) -> int:
        """Legacy API: number of face maps."""

        return 0

    def storage_type_legacy(self) -> str:
        """Legacy helper for adapters that need method-style storage access."""

        return self.storage_type

    def get_times(self) -> npt.NDArray[np.float64]:
        """Legacy API: time values."""

        return self.times()

    def get_time(self, time_step: int) -> float:
        """Legacy API: one-based time value."""

        return float(self.times()[time_step - 1])
    ...
    def get_coord_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: coordinate names."""

        return self.coordinate_names()

    def get_coords(self, time_step: int | None = None) -> npt.NDArray[np.float64]:
        """Legacy API: coordinates."""

        return self.coordinates(
            time=None if time_step is None else time_step - 1, displaced=time_step is not None
        )

    def get_node_id_map(self, file: ExodusFile | None = None) -> npt.NDArray[np.int64]:
        """Legacy API: node ID map.

        If ``file`` is provided, return that component file's local-to-global
        node map. Otherwise return logical global node labels.
        """

        if file is not None:
            file_index = self._file_index(file)
            return self._maps[file_index].node_lid_to_gid

        return self.ids(Entity.NODE)

    def get_element_id_map(self, file: ExodusFile | None = None) -> npt.NDArray[np.int64]:
        """Legacy API: element ID map.

        If ``file`` is provided, return that component file's local-to-global
        element map. Otherwise return logical global element labels.
        """

        if file is not None:
            file_index = self._file_index(file)
            return self._maps[file_index].element_lid_to_gid

        return self.ids(Entity.ELEMENT)

    def get_edge_id_map(self, file: ExodusFile | None = None) -> npt.NDArray[np.int64]:
        """Legacy API: edge ID map."""

        if file is not None:
            file_index = self._file_index(file)
            return self._maps[file_index].edge_lid_to_gid

        return self.ids(Entity.EDGE)

    def get_face_id_map(self, file: ExodusFile | None = None) -> npt.NDArray[np.int64]:
        """Legacy API: face ID map."""

        if file is not None:
            file_index = self._file_index(file)
            return self._maps[file_index].face_lid_to_gid

        return self.ids(Entity.FACE)

    def get_element_block_ids(self) -> npt.NDArray[np.int64]:
        """Legacy API: element block IDs."""

        return self.element_block_ids()

    def get_element_block_id(self, block_iid: int) -> int:
        """Legacy API: block ID from one-based block index."""

        return int(self.element_block_ids()[block_iid - 1])

    def get_element_block_iid(self, block_id: int) -> int | None:
        """Legacy API: one-based block index from block ID."""

        try:
            return _one_based_index(self.element_block_ids(), block_id)
        except ExodusLookupError:
            return None

    def get_element_block(self, block_id: int):
        """Legacy API: element block namespace."""

        from types import SimpleNamespace

        block = self.element_block(block_id)
    ...
    def get_element_conn(self, block_id: int, **_: Any) -> npt.NDArray[np.int64]:
        """Legacy API: element connectivity."""

        return self.element_connectivity(block_id)

    def num_nodes_per_elem(self, block_id: int) -> int:
        """Legacy API: number of nodes per element."""

        return self.element_block(block_id).nodes_per_entity

    def num_elems_in_all_blks(self) -> npt.NDArray[np.int64]:
        """Legacy API: global element counts per block."""

        counts = self._global_variable(VariableName.ELEMENT_BLOCK_COUNT_GLOBAL.value)
        if counts is not None:
            return counts

        return np.asarray(
            [self.element_block(int(block_id)).count for block_id in self.element_block_ids()],
            dtype=np.int64,
        )

    def num_elems_in_blk(self, block_id: int) -> int:
        """Legacy API: global element count in block."""

        counts = self.num_elems_in_all_blks()
        ids = self.element_block_ids()
        index = _one_based_index(ids, block_id) - 1
        return int(counts[index])

    def num_nodes_in_node_set(self, set_id: int) -> int:
        """Legacy API: global node count in node set."""

        counts = self._global_variable(VariableName.NODE_SET_NODE_COUNT_GLOBAL.value)
        if counts is not None:
            ids = self.node_set_ids()
            index = _one_based_index(ids, set_id) - 1
            return int(counts[index])

        return self.node_set(set_id).count

    def num_sides_in_side_set(self, set_id: int) -> int:
        """Legacy API: global side count in side set."""

        counts = self._global_variable(VariableName.SIDE_SET_SIDE_COUNT_GLOBAL.value)
        if counts is not None:
            ids = self.side_set_ids()
            index = _one_based_index(ids, set_id) - 1
            return int(counts[index])

        return self.side_set(set_id).count

    def get_node_set_ids(self) -> npt.NDArray[np.int64]:
        """Legacy API: node set IDs."""

        return self.node_set_ids()

    def get_node_set_id(self, set_iid: int) -> int:
        """Legacy API: node set ID from one-based index."""

        return int(self.node_set_ids()[set_iid - 1])

    def get_node_set_iid(self, set_id: int) -> int | None:
        """Legacy API: one-based node set index."""

        try:
            return _one_based_index(self.node_set_ids(), set_id)
        except ExodusLookupError:
            return None

    def get_node_set(self, set_id: int):
        """Legacy API: node set namespace."""

        from types import SimpleNamespace

        node_set = self.node_set(set_id)
        return SimpleNamespace(
            id=node_set.id,
            iid=node_set.index,
            name=node_set.name,
    ...
    def get_node_set_nodes(self, set_id: int) -> npt.NDArray[np.int64] | None:
        """Legacy API: node set global labels."""

        return self.node_set(set_id).nodes

    def get_node_set_dist_facts(self, set_id: int) -> npt.NDArray[np.float64] | None:
        """Legacy API: node set distribution factors."""

        return self.node_set(set_id).dist_facts

    def get_node_set_params(self, set_id: int):
        """Legacy API: node set params."""
    ...
    def get_side_set_ids(self) -> npt.NDArray[np.int64]:
        """Legacy API: side set IDs."""

        return self.side_set_ids()

    def get_side_set_iid(self, set_id: int) -> int | None:
        """Legacy API: one-based side set index."""

        try:
            return _one_based_index(self.side_set_ids(), set_id)
        except ExodusLookupError:
            return None

    def get_side_set(self, set_id: int):
        """Legacy API: side set namespace."""

        from types import SimpleNamespace

        side_set = self.side_set(set_id)
        return SimpleNamespace(
            id=side_set.id,
            iid=side_set.index,
            name=side_set.name,
    ...
    def get_side_set_elems(self, set_id: int) -> npt.NDArray[np.int64] | None:
        """Legacy API: side set global element labels."""

        return self.side_set(set_id).elems

    def get_side_set_sides(self, set_id: int) -> npt.NDArray[np.int64] | None:
        """Legacy API: side set side numbers."""

        return self.side_set(set_id).sides

    def get_side_set_dist_facts(self, set_id: int) -> npt.NDArray[np.float64] | None:
        """Legacy API: side set distribution factors."""

        return self.side_set(set_id).dist_facts

    def get_side_set_params(self, set_id: int):
        """Legacy API: side set params."""
    ...
    def get_global_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: global variable names."""

        return np.asarray(self.variable_names(Entity.GLOBAL))

    def get_node_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: node variable names."""

        return np.asarray(self.variable_names(Entity.NODE))

    def get_element_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: element variable names."""

        return np.asarray(self.variable_names(Entity.ELEMENT))

    def get_global_variable_values(self, var_name: str) -> npt.NDArray[np.float64]:
        """Legacy API: global variable values."""

        return self.values(var_name, on=Entity.GLOBAL)

    def get_all_global_variable_values(
        self, time_step: int | None = None
    ) -> npt.NDArray[np.float64]:
        """Legacy API: all global variable values."""

        names = self.variable_names(Entity.GLOBAL)
        values = np.column_stack([self.values(name, on=Entity.GLOBAL) for name in names])
    ...
    def get_node_variable_values(
        ...
        """Legacy API: node variable values."""
    ...
    def get_element_variable_values(
        ...
        """Legacy API: element variable values."""
    ...
    def get_node_variable_history(self, var_name: str, node_id: int) -> npt.NDArray[np.float64]:
        """Legacy API: node variable history by global node label."""

        node_ids = self.ids(Entity.NODE)
        row = _one_based_index(node_ids, node_id) - 1
        return self.values(var_name, on=Entity.NODE)[:, row]

    def get_element_variable_history(self, var_name: str, elem_id: int) -> npt.NDArray[np.float64]:
        """Legacy API: element variable history by global element label."""

        element_ids = self.ids(Entity.ELEMENT)
        global_row = _one_based_index(element_ids, elem_id) - 1

        start = 0
        for block_id in self.element_block_ids():
            count = self.element_block(int(block_id)).count
            stop = start + count
            if start <= global_row < stop:
                values = self.values(var_name, on=Entity.ELEMENT, block=int(block_id))
    ...
    def get_edge_block_ids(self) -> npt.NDArray[np.int64]:
        """Legacy API: edge block IDs."""

        return self.edge_block_ids()

    def get_face_block_ids(self) -> npt.NDArray[np.int64]:
        """Legacy API: face block IDs."""

        return self.face_block_ids()

    def get_edge_block_iid(self, block_id: int) -> int | None:
        """Legacy API: one-based edge block index."""

        try:
            return _one_based_index(self.edge_block_ids(), block_id)
        except ExodusLookupError:
            return None

    def get_face_block_iid(self, block_id: int) -> int | None:
        """Legacy API: one-based face block index."""

        try:
            return _one_based_index(self.face_block_ids(), block_id)
        except ExodusLookupError:
            return None

    def get_edge_block(self, block_id: int):
        """Legacy API: edge block namespace."""

        from types import SimpleNamespace

        block = self.edge_block(block_id)
        return SimpleNamespace(
            id=block.id,
            iid=block.index,
            elem_type=block.element_type,
    ...
    def get_face_block(self, block_id: int):
        """Legacy API: face block namespace."""

        from types import SimpleNamespace

        block = self.face_block(block_id)
        return SimpleNamespace(
            id=block.id,
            iid=block.index,
            elem_type=block.element_type,
    ...
    def get_edge_block_conn(self, block_id: int) -> npt.NDArray[np.int64]:
        """Legacy API: edge-block nodal connectivity."""

        return self.edge_connectivity(block_id)

    def get_face_block_conn(self, block_id: int) -> npt.NDArray[np.int64]:
        """Legacy API: face-block nodal connectivity."""

        return self.face_connectivity(block_id)

    def num_edges_in_blk(self, block_id: int) -> int:
        """Legacy API: number of edges in edge block."""

        return self.edge_block(block_id).count

    def num_faces_in_blk(self, block_id: int) -> int:
        """Legacy API: number of faces in face block."""

        return self.face_block(block_id).count

    def num_nodes_per_edge(self, block_id: int) -> int:
        """Legacy API: nodes per edge."""

        return self.edge_block(block_id).nodes_per_entity

    def num_nodes_per_face(self, block_id: int) -> int:
        """Legacy API: nodes per face."""

        return self.face_block(block_id).nodes_per_entity

    def get_edge_set_ids(self) -> npt.NDArray[np.int64]:
        """Legacy API: edge set IDs."""

        return self.edge_set_ids()

    def get_face_set_ids(self) -> npt.NDArray[np.int64]:
        """Legacy API: face set IDs."""

        return self.face_set_ids()

    def get_element_set_ids(self) -> npt.NDArray[np.int64]:
        """Legacy API: element set IDs."""

        return self.element_set_ids()

    def get_edge_set(self, set_id: int):
        """Legacy API: edge set namespace."""

        from types import SimpleNamespace

        set_info = self.edge_set(set_id)
        return SimpleNamespace(
            id=set_info.id,
            iid=set_info.index,
            name=set_info.name,
    ...
    def get_face_set(self, set_id: int):
        """Legacy API: face set namespace."""

        from types import SimpleNamespace

        set_info = self.face_set(set_id)
        return SimpleNamespace(
            id=set_info.id,
            iid=set_info.index,
            name=set_info.name,
    ...
    def get_element_set(self, set_id: int):
        """Legacy API: element set namespace."""

        from types import SimpleNamespace

        set_info = self.element_set(set_id)
        return SimpleNamespace(
            id=set_info.id,
            iid=set_info.index,
            name=set_info.name,
    ...
    def get_edge_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: edge variable names."""

        return np.asarray(self.variable_names(Entity.EDGE))

    def get_face_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: face variable names."""

        return np.asarray(self.variable_names(Entity.FACE))

    def get_node_set_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: node-set variable names."""

        return np.asarray(self.variable_names(Entity.NODE_SET))

    def get_side_set_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: side-set variable names."""

        return np.asarray(self.variable_names(Entity.SIDE_SET))

    def get_edge_set_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: edge-set variable names."""

        return np.asarray(self.variable_names(Entity.EDGE_SET))

    def get_face_set_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: face-set variable names."""

        return np.asarray(self.variable_names(Entity.FACE_SET))

    def get_element_set_variable_names(self) -> npt.NDArray[np.str_]:
        """Legacy API: element-set variable names."""

        return np.asarray(self.variable_names(Entity.ELEMENT_SET))

    def get_edge_variable_values(
        self, block_id: int | None, var_name: str, time_step: int | None = None
    ) -> npt.NDArray[np.float64]:
        """Legacy API: edge variable values."""
    ...
    def get_face_variable_values(
        ...
        """Legacy API: face variable values."""
    ...
    def get_node_set_variable_values(
        ...
        """Legacy API: node-set variable values."""
    ...
    def get_side_set_variable_values(
        ...
        """Legacy API: side-set variable values."""
    ...
    def get_edge_set_variable_values(
        ...
        """Legacy API: edge-set variable values."""
    ...
    def get_face_set_variable_values(
        ...
        """Legacy API: face-set variable values."""
    ...
    def get_element_set_variable_values(
        ...
        """Legacy API: element-set variable values."""
    ...
    def _file_index(self, file: ExodusFile) -> int:
        for index, candidate in enumerate(self._files):
