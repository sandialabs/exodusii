...
.. note::
   ...
   **Limitation:** The exodusii diff requires *matched mesh ordering* —
   both files must have the same node and element IDs.  Coordinate-based
   mesh matching (SEACAS ``exodiff --map``) is not implemented.
